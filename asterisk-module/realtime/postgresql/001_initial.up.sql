...
CREATE TABLE sccp2_realtime_active_generation (
    ...
    generation_id BIGINT NOT NULL UNIQUE
        REFERENCES sccp2_realtime_generations (id)
);

CREATE TABLE sccp2_realtime_sections (
    generation_id BIGINT NOT NULL
        REFERENCES sccp2_realtime_generations (id) ON DELETE CASCADE,
    family TEXT NOT NULL CHECK (family IN ('device', 'line')),
    name TEXT NOT NULL CHECK (length(btrim(name)) > 0),
    section_position BIGINT NOT NULL CHECK (
...
CREATE TABLE sccp2_realtime_fields (
    ...
    CHECK (
        ...
        OR (
            ...
            AND lower(btrim(field_value)) IN (
...
SELECT
    0::BIGINT AS _row_order,
    active.generation_id::TEXT AS _revision,
    1 AS _metadata,
    NULL::TEXT AS name,
    NULL::TEXT AS _field_name,
    NULL::TEXT AS _field_kind,
    NULL::TEXT AS _field_value
FROM sccp2_realtime_active_generation AS active
UNION ALL
SELECT
    section.section_position * 1000000 + field.field_position + 1,
    section.generation_id::TEXT,
    0,
    section.name,
    field.field_name,
    CASE
    ...
    CASE
        ...
        ELSE encode(convert_to(field.field_value, 'UTF8'), 'hex')
    END
