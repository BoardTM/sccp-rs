...
const USAGE: &str = "Usage: chan-sccp2-config-checker <chan_sccp2.conf>\n";
...
fn run(
    ...
    if argument.to_string_lossy().starts_with('-') {
        let _ = write!(stderr, "error: unknown option {argument:?}\n{USAGE}");
        return INVOCATION_ERROR;
    }
    ...
    validate_path(PathBuf::from(argument).as_path(), &mut stdout, &mut stderr)
}

fn validate_path(path: &Path, mut stdout: impl Write, mut stderr: impl Write) -> u8 {
    ...
    match ModuleConfig::parse(&source) {
        Ok(config) => {
